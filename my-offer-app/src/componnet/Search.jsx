...
useEffect(()=>{
  ...
  return (
    <div className='p-20 w-[60%] bg-[#36383d]'>
  ...
  <Box className='border-t-[1px] p-4 mt-5 text-center'>
    ...
    <FormLabel>Experience Code</FormLabel>
      <RadioGroup aria-labelledby="demo-radio-buttons-group-label">
        {experience.map((el,i) => (
        <FormControlLabel value={el} control={<Radio />} label={el} className='mt-[15%] py-4' name="experience_code" key={i} onChange={(e) => setInputs({...inputs,experience_code:e.target.value})}/>
        ))
        }
      </RadioGroup>
      </Box>
